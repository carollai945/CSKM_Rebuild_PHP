<template>
  <div class="page">
    <h2>A04 簽呈申請</h2>
    <div class="page-header"><div></div><button @click="showForm=true">新增簽呈</button></div>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm=false">
      <div class="modal"><h3>A04 簽呈申請</h3>
...
import { petitionsApi } from '@/api/applications'
const rows = ref<Record<string,unknown>[]>([])
const loading = ref(false); const showForm = ref(false)
const form = ref<Record<string,unknown>>({ title:'', content:'' })
async function load() { loading.value=true; const r=await petitionsApi.list(); rows.value=r.data?.data?.data??[]; loading.value=false }
async function save() { await petitionsApi.create(form.value); showForm.value=false; load() }
async function del(id: number) { if(confirm('確認刪除?')) { await petitionsApi.delete(id); load() } }
onMounted(load)
</script>
<style scoped>
.page { padding:1rem }
