...
async function load() { loading.value=true; const r=await approvalsApi.pendingAnnouncement(); rows.value=r.data?.data?.data??r.data?.data??[]; loading.value=false }
async function approve(id: number) { await approvalsApi.approveAnnouncement(id); load() }
async function reject(id: number) { await approvalsApi.rejectAnnouncement(id); load() }
onMounted(load)
</script>
<style scoped>
.page { padding:1rem }
