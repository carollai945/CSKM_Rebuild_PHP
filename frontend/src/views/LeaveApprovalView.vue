...
async function load() {
  loading.value = true
  const r = await approvalsApi.pendingLeave()
...
async function approveSingle(id: number) { await approvalsApi.approveLeave(id); load() }
...
async function batchApprove() { await approvalsApi.batchApproveLeave(selected.value); load() }
async function doReject() {
  if (rejectId.value != null) {
    await approvalsApi.rejectLeave(rejectId.value, rejectReason.value)
    rejectId.value = null
  } else {
    await approvalsApi.batchRejectLeave(selected.value, rejectReason.value)
