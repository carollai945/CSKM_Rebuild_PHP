...
async function load() { loading.value=true; const r=await approvalsApi.pendingLeave(); rows.value=r.data?.data?.data??r.data?.data??[]; loading.value=false }
async function approve(id: number) { await approvalsApi.approveLeave(id); load() }
async function reject(id: number) { await approvalsApi.rejectLeave(id); load() }
onMounted(load)
</script>
<style scoped>
.page { padding:1rem }
