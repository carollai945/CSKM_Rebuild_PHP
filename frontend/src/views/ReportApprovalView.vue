...
async function load() { loading.value=true; const r=await approvalsApi.pendingReport(); rows.value=r.data?.data?.data??r.data?.data??[]; loading.value=false }
async function approve(id: number) { await approvalsApi.approveReport(id); load() }
async function reject(id: number) { await approvalsApi.rejectReport(id); load() }
onMounted(load)
</script>
<style scoped>
.page { padding:1rem }
