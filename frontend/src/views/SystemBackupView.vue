<template>
  <div class="page">
    ...
    <div class="card">
      ...
      <table v-else>
        <thead><tr><th>檔案名稱</th><th>建立時間</th></tr></thead>
        <tbody>
          <tr v-for="r in rows" :key="r.filename as string">
            <td>{{r.filename}}</td><td>{{r.created_at}}</td>
          </tr>
        </tbody>
      </table>
    </div>
...
async function load() { loading.value=true; const r=await systemApi.listBackups(); rows.value=r.data?.data??[]; loading.value=false }
async function createBackup() { await systemApi.createBackup(); alert('備份成功'); load() }
onMounted(load)
</script>
<style scoped>
.page { padding:1rem }
