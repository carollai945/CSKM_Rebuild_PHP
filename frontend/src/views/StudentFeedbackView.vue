<template>
  <div class="page">
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal">
        <h3>{{ editId ? '編輯' : '新增' }}學員意見管理</h3>
        <form @submit.prevent="save">
          <div><label>學員ID</label><input v-model="form.student_id" placeholder="學員ID" /></div>
          <div><label>類別</label><input v-model="form.category" placeholder="類別" /></div>
...
async function load() { loading.value = true; const r = await studentFeedbacksApi.list(); rows.value = r.data?.data?.data ?? r.data?.data ?? []; loading.value = false }
function edit(row: Record<string,unknown>) { editId.value = row.id as number; form.value = {...row}; showForm.value = true }
async function save() { if (editId.value) await studentFeedbacksApi.update(editId.value, form.value); else await studentFeedbacksApi.create(form.value); showForm.value = false; editId.value = null; form.value = {}; load() }
async function remove(id: number) { if (confirm('確認刪除?')) { await studentFeedbacksApi.delete(id); load() } }
onMounted(load)
</script>
<style scoped>
.page { padding: 1rem }
