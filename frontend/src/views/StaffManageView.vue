    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal">
        ...
        <form @submit.prevent="save">
          ...
          <div><label>到職日</label><input v-model="form.join_date" placeholder="到職日" /></div>
...
async function remove(id: number) { if (confirm('確認刪除?')) { await staffApi.delete(id); load() } }
onMounted(load)
</script>
<style scoped>
.page { padding: 1rem }
