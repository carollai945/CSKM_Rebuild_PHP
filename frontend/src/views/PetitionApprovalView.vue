...
async function load() {
  loading.value = true
  const r = await approvalsApi.pendingPetition()
...
async function approveSingle(id: number) { await approvalsApi.approvePetition(id); load() }
...
async function batchApprove() { await approvalsApi.batchApprovePetition(selected.value); load() }
async function doReject() {
  if (rejectId.value != null) {
    await approvalsApi.rejectPetition(rejectId.value, rejectReason.value)
    rejectId.value = null
  } else {
    await approvalsApi.batchRejectPetition(selected.value, rejectReason.value)
