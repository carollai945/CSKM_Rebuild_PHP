...
async function load() { loading.value = true; const r = await regionsApi.list(); rows.value = r.data?.data?.data ?? r.data?.data ?? []; loading.value = false }
function edit(row: Record<string,unknown>) { editId.value = row.id as number; form.value = {...row}; showForm.value = true }
async function save() { if (editId.value) await regionsApi.update(editId.value, form.value); else await regionsApi.create(form.value); showForm.value = false; editId.value = null; form.value = {}; load() }
async function remove(id: number) { if (confirm('確認刪除?')) { await regionsApi.delete(id); load() } }
onMounted(load)
</script>
<style scoped>
.page { padding: 1rem }
