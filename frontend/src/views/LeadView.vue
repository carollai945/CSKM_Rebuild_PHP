...
async function load() { loading.value = true; const r = await leadsApi.list(); rows.value = r.data?.data?.data ?? r.data?.data ?? []; loading.value = false }
function edit(row: Record<string,unknown>) { editId.value = row.id as number; form.value = {...row}; showForm.value = true }
async function save() { if (editId.value) await leadsApi.update(editId.value, form.value); else await leadsApi.create(form.value); showForm.value = false; editId.value = null; form.value = {}; load() }
