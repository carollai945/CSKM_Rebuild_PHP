<template>
  <div class="page">
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal">
        <h3>{{ editId ? '編輯' : '新增' }}教室管理</h3>
        <form @submit.prevent="save">
          <div><label>名稱</label><input v-model="form.name" placeholder="名稱" /></div>
          <div><label>容量</label><input v-model="form.capacity" placeholder="容量" /></div>
...
async function load() { loading.value = true; const r = await classroomsApi.list(); rows.value = r.data?.data?.data ?? r.data?.data ?? []; loading.value = false }
function edit(row: Record<string,unknown>) { editId.value = row.id as number; form.value = {...row}; showForm.value = true }
async function save() { if (editId.value) await classroomsApi.update(editId.value, form.value); else await classroomsApi.create(form.value); showForm.value = false; editId.value = null; form.value = {}; load() }
async function remove(id: number) { if (confirm('確認刪除?')) { await classroomsApi.delete(id); load() } }
onMounted(load)
</script>
<style scoped>
.page { padding: 1rem }
